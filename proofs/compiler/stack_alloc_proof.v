...
Proof.
  ...
  + case: (hwfr) => hwfsr hwfst hval hptr; split.
    + have [_ _ ->] := set_wordP hwf haddr hset.
      by move=> ?? /=; apply hwfsr.
    + by apply (wfr_STATUS_set_word hwfr hset hwf hwfs).
...
Lemma valid_state_set_move table rmap se m0 s1 s2 x sr status pk v :
  ...
  valid_state table (set_move rmap x sr status) se m0 (with_vm s1 (evm s1).[x <- v]) s2.
Proof.
...
Proof.
  move=> hvs hwf haddr hwfs hlx heqval htr.
  have /wf_locals /= hlocal := hlx.
  case:(hvs) => hscs hvalid hdisj hincl hincl2 hunch hrip hrsp heqvm hwft hwfr heqmem hglobv htop.
  constructor=> //=.
...
Lemma check_gvalid_set_stack_ptr rmap se s2 s ws cs f y sry statusy :
  wfr_PTR rmap se s2 ->
  Sv.In f pmap.(vnew) ->
...
Proof.
  ...
  case hsry: Mvar.get => [sr|//] [? <-]; subst sr.
  have hneq: f <> y.(gv).
  + by move /(var_region_not_new hptr) : hsry; congruence.
  rewrite get_var_status_set_word_status.
  have /eqP /negPf -> /= := hneq.
  case: eqP => heqr /=.
  + by left; split.
...
(* The lemma manipulates [set_stack_ptr (set_move ...)], rather than
   [set_stack_ptr] alone.
*)
Lemma check_gvalid_set_stack_ptr table rmap se m0 s1 s2 s ws z f y sry statusy x sr status :
  valid_state table rmap se m0 s1 s2 ->
  ~ Sv.In x pmap.(vnew) ->
  Sv.In f pmap.(vnew) ->
  check_gvalid (set_stack_ptr (set_move rmap x sr status) s ws z f) y = Some (sry, statusy) ->
  exists statusy',
    check_gvalid (set_move rmap x sr status) y = Some (sry, statusy') /\
      (¬ is_glob y → f ≠ gv y) ∧ statusy' = statusy. (*
        ~ is_glob y -> f <> gv y &
        bytesy =
          if (sub_region_stkptr s ws z).(sr_region) != sry.(sr_region) then statusy'
...
Proof.
  move=> hptr hlocal hpaddr hreadeq hneq.
  ...
  + have hneqf := hlocal.(wfs_distinct) hly hneq.
    ...
    have := var_region_not_new hptr heq (wf_locals hly).(wfs_new). done.
...
Lemma valid_state_set_move_stkptr table rmap se m0 s1 s2 x sr addr s ofs ws z f paddr status v :
  ...
  sub_region_addr se (sub_region_stkptr s ws z) = ok paddr ->
  read (emem s2) Aligned paddr Uptr = ok addr ->
  truncatable true (vtype x) v ->
  eq_sub_region_val x.(vtype) se (emem s2) sr status (vm_truncate_val (vtype x) v) ->
  valid_state
    (remove_binding table x) (set_move rmap x sr status)
    se m0 (with_vm s1 (evm s1).[x <- v]) s2.
Proof.
  move=> hvs hwf haddr hwfs hlx hpaddr hreadptr htr heqval.
  have /wf_locals hlocal := hlx.
  have hwf' := sub_region_stkptr_wf se hlocal.
  case:(hvs) => hscs hvalid hdisj hincl hincl2 hunch hrip hrsp heqvm hwft hwfr heqmem hglobv htop.
  constructor=> //=.
...
(*   eq_sub_region_val x.(vtype) se (emem s2) sr status (vm_truncate_val (vtype x) v) -> *)
  let rmap :=
    let rmap := set_stack_ptr rmap s ws cs f in
    {| var_region := Mvar.remove rmap.(var_region) x;
       region_var := rmap.(region_var) |}
  in
  valid_state
    table rmap
    se m0 s1 (with_mem s2 mem2).
Proof.
  move=> hvs hwf haddr hwfs hlx hpaddr hss hvalideq hreadeq.
...
(*   have hwfs' := sub_region_at_ofs_0_wf hwfs. *)
  ...
  + case: (hwfr) => hwfsr hwfst hval hptr; split=> //.
    + move=> y sry /=.
      rewrite Mvar.removeP. case: eqP => // _. apply hwfsr.
    + (* FIXME: clean *)
      apply wfr_STATUS_alt.
      move /wfr_STATUS_alt in hwfst.
      move=> /= r y.
...
Fixpoint no_glob e :=
  ...
  | Pvar x => ~~ is_glob x
  | Pget _ _ _ x e | Psub _ _ _ x e => ~~ is_glob x && no_glob e
  | Pload _ _ _ e => no_glob e
  | Papp1 _ e => no_glob e
  | Papp2 _ e1 e2 => no_glob e1 && no_glob e2
  | PappN _ es => all no_glob es
  | Pif _ b e1 e2 => [&& no_glob b, no_glob e1 & no_glob e2]
  end.

Lemma no_globP wdb gd s e :
  no_glob e ->
  sem_pexpr wdb gd s e = sem_pexpr wdb [::] s e.
Proof.
  move: e.
  suff: [elaborate
    (forall e, no_glob e -> sem_pexpr wdb gd s e = sem_pexpr wdb [::] s e) /\
    (forall es, all no_glob es -> sem_pexprs wdb gd s es = sem_pexprs wdb [::] s es)].
  + by case.
  apply pexprs_ind_pair; split=> //=.
  + by move=> e ihe es ihes /andP [/ihe -> /ihes ->].
  + move=> x hnglob.
    by rewrite !get_gvar_nglob //.
  + move=> al aa ws x e ihe /andP [hnglob /ihe ->].
    by rewrite /on_arr_var !get_gvar_nglob //.
  + move=> aa ws len x e ihe /andP [hnglob /ihe ->].
    by rewrite /on_arr_var !get_gvar_nglob //.
  + by move=> al ws x e ihe /ihe ->.
  + by move=> o e ihe /ihe ->.
  + by move=> o e1 ihe1 e2 ihe2 /andP [/ihe1 -> /ihe2 ->].
  + rewrite -/(sem_pexprs _ _ _).
    by move=> o es ihes /ihes ->.
  by move=> ty b ihb e1 ihe1 e2 ihe2 /and3P [/ihb -> /ihe1 -> /ihe2 ->].
Qed.

Lemma symbolic_of_pexpr_no_glob clone table e table' e' :
  symbolic_of_pexpr clone table e = Some (table', e') ->
  no_glob e.
Proof.
  move: e table e' table'.
  suff:
    (forall e table e' table',
      symbolic_of_pexpr clone table e = Some (table', e') → no_glob e) /\
    (forall es table es' table',
      fmapo (symbolic_of_pexpr clone) table es = Some (table', es') → all no_glob es).
  + by case.
  apply pexprs_ind_pair; split=> //=.
  + move=> e ihe es ihes table es' table'.
    apply: obindP => -[table1 e1] /ihe ->.
    by apply: obindP => -[_ _] /ihes -> _.
  + move=> x table e' table'.
    rewrite is_lvar_is_glob.
    by case: is_glob.
  + move=> o e ihe table e' table'.
    by apply: obindP => -[_ _] /ihe -> _.
  + move=> o e1 ihe1 e2 ihe2 table e' table'.
    apply: obindP => -[table1 e1'] /ihe1 ->.
    by apply: obindP => -[_ _] /ihe2 -> _.
  + move=> o es ihes table e' table'.
    by apply: obindP => -[_ _] /ihes -> _.
  move=> ty b ihb e1 ihe1 e2 ihe2 table e' table'.
  apply: obindP => -[table1 b'] /ihb ->.
  apply: obindP => -[table2 e1'] /ihe1 ->.
  by apply: obindP => -[_ _] /ihe2 -> _.
Qed.
*)
...
Lemma test table rmap se m0 s1 s2 r :
  valid_state table rmap se m0 s1 s2 ->
  valid_state (remove_binding_lval table r) rmap se m0 s1 s2.
Proof.
  move=> hvs.
  case:(hvs) => hscs hvalid hdisj hincl hincl2 hunch hrip hrsp heqvm hwft hwfr heqmem hglobv htop.
  split=> //.
  ...
  + move=> x.
    ...
    split=> //=.
    + move=> y ey /=.
      rewrite Mvar.removeP. case: eqP => // _. eauto.
    move=> y ey vy /=.
    rewrite Mvar.removeP. case: eqP => // _. eauto.
  + move=> _ _ _ x _.
    case: hwft => h1 h2 h3.
...
Proof.
  ...
  + case: e he He => //=.
    + t_xrbindP=> y hgety vpky hkindy.
      ...
      + assert (hwfsy := check_gvalid_wf_status wfr_status hgvalidy).
        apply wf_status_vm_uincl. done. done.
      + by rewrite /= truncate_word_u.
      + by rewrite with_vm_same.
      ...
      by rewrite -(type_of_get_gvar_array hgety).
    move=> aa ws len y e.
    apply: on_arr_gvarP => n' a' hyty hgety.
    apply: rbindP=> i ok_i.
    ...
    (* TODO: move as a section lemma? *)
    have clone_ty: forall x n, vtype (clone fresh_var_ident x n) = vtype x.
    + done.
    ...
    case: andP.
    + move=> /= [h0 /eqP heq].
      t_xrbindP=> e' halloc -[ey' ofsy'] haddr <- <- <- _ <- <-.
      have [wey ok_wey haddry] :=
        addr_from_vpk_pexprP true hvs hwfpky hpky hwfy haddr.
      move: ok_i; t_xrbindP=> ve ok_ve ok_i.
      have := alloc_eP hvs halloc ok_ve.
      rewrite /truncate_val /= ok_i /= => /(_ _ erefl) [] ve' [] ok_ve'.
      t_xrbindP=> i' ok_i' ?; subst i'.
      exists vme, wey, (wrepr Uptr (i * mk_scale aa ws + ofsy')); split=> //.
      + rewrite heq (type_of_get_gvar_array hgety) /= -(type_of_get_gvar_array hgety).
        by apply (wf_sub_region_vm_uincl huincl).
      + assert (hwfsy := check_gvalid_wf_status wfr_status hgvalidy).
        apply wf_status_vm_uincl. done. done.
      + have h: sem_pexpr true [::] s2 e' >>= to_int = ok i.
        + by rewrite ok_ve' /= ok_i'.
        by rewrite (mk_ofsP aa ws ofsy' h) /= truncate_word_u.
      + have [ve1 ok_ve1 incl_ve1] := hsem _ _ ok_ve.
        have /= {}ok_i := of_value_uincl_te (ty:=sint) incl_ve1 ok_i.
        have h: sem_pexpr true [::] (with_vm se vme) e1 >>= to_int = ok i.
        + by rewrite ok_ve1 /= ok_i.
        have := mk_ofs_intP aa ws h.
        rewrite (eq_exprP true _ _ h0) /= => -[<-] /=.
        by apply (sub_region_addr_vm_uincl huincl).
      split=> //.
      assert (hval := wfr_val hgvalidy hgety).
      have [hread hty] := hval.
      move=> off /= ofs w.
      rewrite (sub_region_addr_vm_uincl huincl haddry) => -[?]; subst ofs.
      move=> off_valid ok_w.
      have /= hwfs := [elaborate check_gvalid_wf_status wfr_status hgvalidy].
      have /(valid_offset_vm_uincl _ huincl hwfs) off_valid' := off_valid.
      have := hread off _ w haddry off_valid'.
      apply.
      rewrite /=.
      have /= := WArray.get_sub_get8 ok_a'' off.
      rewrite ok_w. case: ifP => // _.
      have [ve1 ok_ve1 incl_ve1] := hsem _ _ ok_ve.
      have /= {}ok_i := of_value_uincl_te (ty:=sint) incl_ve1 ok_i.
      have h: sem_pexpr true [::] (with_vm se vme) e1 >>= to_int = ok i.
      + by rewrite ok_ve1 /= ok_i.
      have := mk_ofs_intP aa ws h.
      by rewrite (eq_exprP true _ _ h0) /= => -[<-] /=.
    move=> _.
    t_xrbindP=> e' halloc -[ey' ofsy'] haddr <- <- <- _ <- <-.
      have [wey ok_wey haddry] :=
        addr_from_vpk_pexprP true hvs hwfpky hpky hwfy haddr.
      move: ok_i; t_xrbindP=> ve ok_ve ok_i.
(*       have := sub_region_addr_offset hwfy. *)
      have := alloc_eP hvs halloc ok_ve.
      rewrite /truncate_val /= ok_i /= => /(_ _ erefl) [] ve' [] ok_ve'.
      t_xrbindP=> i' ok_i' ?; subst i'.
      exists vme, wey, (wrepr Uptr (i * mk_scale aa ws + ofsy')); split=> //.
    + have [ve1 ok_ve1 incl_ve1] := hsem _ _ ok_ve.
      ...
      have h: sem_pexpr true [::] (with_vm se vme) e1 >>= to_int = ok i.
      + by rewrite ok_ve1 /= ok_i.
      have {}h := mk_ofs_intP aa ws h.
      (* do we need a diff version of mk_ofs_intP to remove this ugly thing? *)
      have {}h: sem_pexpr true [::] (with_vm se vme) (mk_ofs_int aa ws e1) >>= to_int = ok (i * mk_scale aa ws).
      + by rewrite h.

      have := sub_region_at_ofs_wf (wf_sub_region_vm_uincl huincl hwfy) h.
      have h_ :=  (WArray.get_sub_bound ok_a'').
      rewrite (type_of_get_gvar_array hgety) /=.
      move=> /(_ (sarr (Z.to_pos (arr_size ws len))) h_). done.
    + by case: get_sub_status.
    + have h: sem_pexpr true [::] s2 e' >>= to_int = ok i.
      + by rewrite ok_ve' /= ok_i'.
      by rewrite (mk_ofsP aa ws ofsy' h) /= truncate_word_u.
    + have [ve1 ok_ve1 incl_ve1] := hsem _ _ ok_ve.
      ...
      have {}h: sem_pexpr true [::] (with_vm se vme) (mk_ofs_int aa ws e1) >>= to_int = ok (i * mk_scale aa ws).
      + by rewrite h.
      have := sub_region_addr_offset (wf_sub_region_vm_uincl huincl hwfy) h.
      have h_ :=  (WArray.get_sub_bound ok_a'').
      rewrite (type_of_get_gvar_array hgety) /=.
      move=> /(_ (sarr (Z.to_pos (arr_size ws len))) _ h_).
      move=> /(_ _ (sub_region_addr_vm_uincl huincl haddry)).
      rewrite wrepr_add (GRing.addrC (wrepr _ _)) GRing.addrA.
      by rewrite /= Z2Pos.id //.
    split=> //.
    move=> off ofs w ok_ofs hvalid ok_w.
    move: hvalid; case hstatus: get_sub_status => //= _.
    assert (hval := wfr_val hgvalidy hgety).
    have [hread hty] := hval.
    have := hread _ _ _ haddry.
    have [ve1 ok_ve1 incl_ve1] := hsem _ _ ok_ve.
      have /= {}ok_i := of_value_uincl_te (ty:=sint) incl_ve1 ok_i.
      have h: sem_pexpr true [::] (with_vm se vme) e1 >>= to_int = ok i.
      + by rewrite ok_ve1 /= ok_i.
      ...
      move=> /(_ _ (sub_region_addr_vm_uincl huincl haddry)).
      rewrite /= Z2Pos.id //.
      rewrite ok_ofs => -[?]; subst ofs.
      rewrite -GRing.addrA -wrepr_add. apply.
      have /= hwfs := [elaborate check_gvalid_wf_status wfr_status hgvalidy].
      apply (valid_offset_vm_uincl _ huincl hwfs).
      apply: (get_sub_statusP hstatus).
      apply (wf_status_vm_uincl huincl). done.
      rewrite /sem_slice /=. rewrite h /=. reflexivity.
      rewrite /offset_in_concrete_slice /= !zify.
      have /= := get_val_byte_bound ok_w. rewrite Z2Pos.id. lia.
      rewrite arr_sizeE. have := wsize_size_pos ws. lia.
      have /= := WArray.get_sub_get8 ok_a'' off.
      move: ok_w => /= ->.
      case: ifP => // _.
      (*
      ...
      move: ok_w. rewrite /=. WArray.get_sub
      rewrite /arr_size.
      have := @ge0_arr_size ws len. have /= := get_val_byte_bound ok_w.
      
      Z.to_pos 0%Z
      Z.to_pos
      apply.
      
      move: hstatus. rewrite /get_sub_status. case: (statusy') => //.
      move=> i' hget.
      have := get_sub_statusP hget.
    
     get_sub_status get_sub_interval
    have /wfr_val := hgvalidy. valid_offset_vm_uincl y
      move=> /(_ _ haddry).
      rewrite /sub_region_at_ofs /= /sub_region_addr /=.
    
    rewrite (type_of_get_gvar_array hgety). /= -(type_of_get_gvar_array hgety).
        by apply (wf_sub_region_vm_uincl huincl).
    
      
    case hsub: sub_region_status_at_ofs => [sry'' statusy''].
    t_xrbindP=> -[ey' ofsy'] haddr /= <- <- <- _ _ _.
    exists vme; split=> //.
    + 
    rewrite -(type_of_get_gvar_array hgety).
     by apply wf_sub_region_vm_uincl
    move=> hs.
    rewrite /sub_region_status_at_ofs.
    case: andP.
    + move=> heq.
    ...
    assert (hwfy := check_gvalid_wf wfr_wf hgvalidy). simpl in hwfy.
    move: h; rewrite /get_symbolic_of_pexpr. move=> /o2rP h.
    have hty: forall x n, vtype (clone fresh_var_ident x n) = vtype x.
    + done.
    have [vme [h1 h2 h3]] := wf_table_symbolic_of_pexpr hty h hvs.(vs_wf_table).
    move: ok_i; t_xrbindP=> vi ok_vi ok_i.
    have [vi2 ok_vi2 huincl] := h3 _ _ ok_vi.
    have H: Let x := sem_pexpr true [::] (with_vm se vme) se1 in to_int x = ok i.
    + rewrite ok_vi2 /=.
      have /= := of_value_uincl_te (ty:=sint) huincl ok_i. done.
    have {}H := mk_ofs_intP aa ws H.

    have wf_sub_region_vm_uincl:
      forall se vme sr ty,
      se.(evm) <=1 vme ->
      wf_sub_region se sr ty ->
      wf_sub_region (with_vm se vme) sr ty.
    + move=> ???? huincl'.
      case=> k1 k2. split=> //=.
      case: k2 => cs ok_cs wf_cs.
      exists cs => //.
      apply (sem_zone_vm_uincl huincl' ok_cs).

  have hwfy' := wf_sub_region_vm_uincl _ _  _ _ h2 hwfy.
...
(*     have := mk_ofs_intP aa ws ok_i. *)
  ...
  move=> [vme [wey [wofsy [huincl hwft hwfy hwfsy ok_wey ok_wofsy haddr heqval]]]].
...
*)
  ...
  + case:(hvs) => hscs hvalid hdisj hincl hincl2 hunch hrip hrsp heqvm hwft_ hwfr heqmem hglobv htop.
    split=> //.
    ...
    + t_xrbindP=> s ofs' ws z sc hlx hlocal heqsub <- <- <-.
      ...
      + (* valid_state update *)
        apply: valid_state_set_move.
        + apply test. done.
        + rewrite hty. done.
        + apply hlx.
        + by simpl; symmetry.
        + done.
        + rewrite htreq hty. done.
      done.
    ...
    + move=> p hlx hlocal.
      ...
      move=> /= [<- <- <-].
      set vp := Vword (wey + wofsy)%R.
      have /(_ (with_vm s2 (evm s2).[p <- vp])) []:= (mov_ofsP hsaparams rip P'_globs ok_wey ok_wofsy Hmov_ofs).
      + rewrite /=.
        have htr : type_of_val vp = vtype (with_var x p) by rewrite hlocal.(wfr_rtype).
      ...
      have := valid_state_set_move_regptr hvs' _ haddr hwfsy hlx. apply.
      + rewrite hty. done.
      + rewrite hty. done.
      + rewrite hty. rewrite vm_truncate_val_eq //.
    ...
    + rewrite /is_nop.
      ...
      exists s2, vme; split=> //.
      + by constructor.
      have := valid_state_set_move_stkptr hvs'. apply.
      + rewrite hty. done.
      + apply haddr.
      + done.
      + apply hlx.
      ...
      + have /wfr_ptr := heq; rewrite hlx => -[_ [[<-] hpk]]. apply hpk.
        ...
        rewrite (sub_region_beq_addr _ heqsub). done.
      + done. (* do we need h ? *)
      rewrite hty. rewrite vm_truncate_val_eq //.
    move=> _.
    rewrite /get_addr.
    case Hmov_ofs: (sap_mov_ofs saparams) => [ins| //] /=.
    move=> [<- <- <-].
    have hvp: validw (emem s2) Aligned addr Uptr.
    + have := validw_sub_region_addr_ofs hvs hwf ok_addr.
      have h_: 0 <= 0 /\ wsize_size Uptr <= size_of (sword Uptr). simpl. by lia.
      move=> /(_ _ _ _ h_).
      rewrite wrepr0 GRing.addr0. apply.
      by apply (is_align_sub_region_stkptr hlocal ok_addr).
      have /writeV -/(_ (wey + wofsy)%R) [mem2 hmem2] := hvp.
      have /(_ (with_mem s2 mem2)) []:= mov_ofsP hsaparams rip P'_globs ok_wey ok_wofsy Hmov_ofs.
      + rewrite /= /get_var vs_rsp /= !truncate_word_u /=.
      move: ok_addr; rewrite (sub_region_addr_stkptr _ hlocal) => -[->].
      by rewrite hmem2.
    move=> vm2 hsem heq1.
    exists (with_vm (with_mem s2 mem2) vm2), vme; split => //.
    apply valid_state_vm_eq => //.
    set rmap_ := set_stack_ptr rmap1 s ws z f.
    set rmap__ := {| var_region := Mvar.remove rmap_.(var_region) x;
                     region_var := rmap_.(region_var) |}.
    have: set_stack_ptr (set_move rmap1 x sry statusy) s ws z f =
      set_move rmap__ x sry statusy.
    + admit.
    move=> ->.
(*     have: valid_state table1' rmap_ (with_vm se vme) m0 s1 ( *)
(*     valid_statde_set_stack_ptr *)
    have hvs0: valid_state table1' rmap__ (with_vm se vme) m0 s1 (with_mem s2 mem2).
    + have := valid_state_set_stack_ptr hvs'. apply.
      + rewrite hty. apply hwfy.
      + apply haddr.
      + apply hwfsy.
      + apply hlx.
      + apply ok_addr.
    ...
    have := valid_state_set_move_stkptr. apply.
    + rewrite hty. done.
    + apply haddr.
    + done.
    + apply hlx.
    + apply ok_addr.
    + simpl. apply (writeP_eq hmem2).
    + done.
    rewrite hty vm_truncate_val_eq //=.
    have := eq_sub_region_val_same_region _ _ _ _ _ _ _ heqval.
    have := eq_sub_region_val_distinct_regions _ _ _ _ _ _ heqval. apply.
    + apply (wf_sub_region_vm_uincl huincl). apply hwf.
    + apply (sub_region_addr_vm_uincl huincl). done.
    + done.
    + simpl.  admit.
    apply hwf.
    case: heqval => [hreadeq hty'].
    split=> //.
    move=> hcheck ofs w ok_ofs hvalid ok_w.
    have := hreadeq hcheck ofs w ok_ofs hvalid ok_w. move=> <-.
    apply (writeP_neq _ hmem2). addr sry set_stack_ptr set_word_status valid_state_set_move_stkptr
      + have /wfr_ptr := heq; rewrite hlx => -[_ [[<-] hpk]]. apply hpk.
        done. done.
        rewrite (sub_region_beq_addr _ heqsub). done.
      + done. (* do we need h ? *)
    ...
    have {hi2} [vm2 [mem2 [hsemi heq1 hss hvalideq hreadeq hreadptr]]]:
      ...
      [/\ sem_i P' rip s2 i2 (with_vm (with_mem s2 mem2) vm2),
          ...
          validw mem2 =3 validw (emem s2),
          (forall al p ws,
            disjoint_range addr Uptr p ws ->
            read mem2 al p ws = read (emem s2) al p ws) &
          read mem2 Aligned addr Uptr =
            ok (wey + wofsy)%R].
    + move: hi2.
      case: ifP.
      + rewrite /is_nop.
