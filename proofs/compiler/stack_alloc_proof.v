...
Lemma incl_merge_interval i1 i1' i2 i2' i i' :
  incl_interval i1 i1' ->
  incl_interval i2 i2' ->
...
(* symbolic_slice_beq does not return a Prop, is_true is inserted.
   Due to the coercion, setoid does not work well. We define this boring lemma
   to make it work better. *)
Instance Equivalence_eq_compat {A} (eqA : rel A) :
  Equivalence eqA -> Proper (eqA ==> eqA ==> eq) eqA.
Proof.
  move=> hequiv.
...
(* The reverse inclusion is also true, but we need this direction only. *)
Lemma symbolic_slice_beq_add_sub_interval s1 s2 i i1 :
  symbolic_slice_beq s1 s2 ->
  add_sub_interval i s1 = Some i1 ->
  exists2 i2,
    add_sub_interval i s2 = Some i2 &
    incl_interval i2 i1.
Proof.
  move=> heq.
  elim: i i1 => [|s i ih] i1 /=.
  + move=> [<-].
    eexists; first by reflexivity.
    by rewrite /= (symbolic_slice_beq_sym' heq) /=.
  have ->: symbolic_slice_beq s2 s = symbolic_slice_beq s1 s.
  + by rewrite -> heq.
  have ->: symbolic_slice_ble s2 s = symbolic_slice_ble s1 s.
  + by rewrite -> heq.
  have ->: symbolic_slice_ble s s2 = symbolic_slice_ble s s1.
  + by rewrite -> heq.
  ...
  + move=> [<-].
    eexists; first by reflexivity.
    rewrite /= (symbolic_slice_beq_sym' heq) /=.
    by apply (incl_interval_cons s1 (s :: i)).
  case hle2: (odflt _ _) => //.
  apply: obindP => {}i1 hadd [<-].
  have [i2 -> hincl] := ih _ hadd.
  eexists; first by reflexivity.
  rewrite /= symbolic_slice_beq_refl /=.
  by apply: incl_interval_trans (incl_interval_cons _ _).
Qed.

(* We have the reverse inclusion by symmetry of symbolic_zone_beq *)
Lemma symbolic_zone_beq_clear_status z1 z2 status :
  symbolic_zone_beq z1 z2 ->
  incl_status
    (odflt Unknown (clear_status status z1))
    (odflt Unknown (clear_status status z2)).
Proof.
  case: z1 z2 => [|s1 _] [|s2 _] //= /andP [heq _].
  case: status => [||i] /=.
  + by rewrite (symbolic_slice_beq_sym' heq).
  + by [].
  case hadd1: add_sub_interval => [i1|//] /=.
  by have [i2 -> heq'] /= := symbolic_slice_beq_add_sub_interval heq hadd1.
Qed.

(* We have the reverse inclusion by symmetry of symbolic_zone_beq *)
Lemma symbolic_zone_beq_clear_status' z1 z2 status status1 :
  symbolic_zone_beq z1 z2 ->
  clear_status status z1 = Some status1 ->
  exists2 status2,
    clear_status status z2 = Some status2 &
    incl_status status1 status2.
Proof.
  case: z1 z2 => [|s1 _] [|s2 _] //= /andP [heq _].
  case: status => [||i] /=.
  ...
  have [i2 -> heq'] := symbolic_slice_beq_add_sub_interval heq hadd1.
  by eexists; first by reflexivity.
Qed.

Lemma incl_interval_add_sub_interval_compat se s i1 i2 i1' :
  wf_interval se i1 ->
  wf_interval se i1' ->
  add_sub_interval i1 s = Some i2 ->
  incl_interval i1' i1 ->
  exists2 i2',
    ...
    incl_interval i2' i2.
Proof.
  elim: i1 i1' i2 => [|s1 i1 ih1] i1' i2 /= wf_i1 wf_i1'.
  + move=> [<-].
    case: i1' {wf_i1'} => [|//] /= _.
    eexists; first by reflexivity.
    by apply incl_interval_refl.
  ...
  + move=> [<-].
    ...
    case heq': symbolic_slice_beq.
    + eexists; first by reflexivity.
      by rewrite /= hin hincl.
    case hle1: (odflt _ _).
    + eexists; first by reflexivity.
      by rewrite /= heq hin hincl /=. (*
    case hle2: (odflt _ _).
    + add_sub_interval
      have: symbolic_slice_beq s1' s
    ...
    move=> h1.
    case hle1: (odflt _ _).
    + eexists; first by reflexivity. simpl. rewrite h2.
      rewrite h /=. rewrite /= in h1. rewrite h1. rewrite orbT. done.
    case hle2: (odflt _ _) => //. admit.
...
Lemma incl_status_clear_status_compat se status1 status2 z status1' :
  wf_status se status1 ->
  wf_status se status2 ->
  incl_status status1 status2 ->
  clear_status status1 z = Some status1' ->
  exists2 status2',
    clear_status status2 z = Some status2' &
    incl_status status1' status2'.
Proof.
  move=> hwfs1 hwfs2 hincl.
  case: z => [//|s _] /=.
...
Lemma incl_status_clear_status_map_aux_compat se rmap1 rmap2 status1 status2 z x status1' :
  incl rmap1 rmap2 ->
  wf_status se status1 ->
  wf_status se status2 ->
  incl_status status1 status2 ->
  clear_status_map_aux rmap1 z x status1 = Some status1' ->
  exists2 status2',
    clear_status_map_aux rmap2 z x status2 = Some status2' &
    incl_status status1' status2'.
Proof.
  move=> hinclr hwfs1 hwfs2 hincls.
  rewrite /clear_status_map_aux.
...
Lemma incl_status_map_clear_status_map_compat se rmap1 rmap2 sm1 sm2 z :
  incl rmap1 rmap2 ->
  (forall x status, Mvar.get sm1 x = Some status -> wf_status se status) ->
  (forall x status, Mvar.get sm2 x = Some status -> wf_status se status) ->
  incl_status_map sm1 sm2 ->
  incl_status_map (clear_status_map rmap1 z sm1) (clear_status_map rmap2 z sm2).
Proof.
  move=> hinclr hwfst1 hwfst2 /Mvar.inclP hincl.
  apply /Mvar.inclP => x.
  rewrite /clear_status_map !Mvar.filter_mapP.
  case heq1: (Mvar.get sm1 x) (hincl x) => [status1|//] /=.
  case heq2: Mvar.get => [status2|//] /=.
  move=> hincls.
  case hclear1: clear_status_map_aux => [status1'|//] /=.
  by have [status2' -> hincls'] :=
       incl_status_clear_status_map_aux_compat hinclr
...
Proof.
  ...
  apply (incl_status_map_clear_status_map_compat (se:=se) _ hincl).
  + move=> x status.
    case heq1 : Mr.get => [sm1|] /=; last by rewrite Mvar.get0.
    by apply (hwfst1 _ _ _ _ heq1).
  + move=> x status.
    case heq2 : Mr.get => [sm2|] /=; last by rewrite Mvar.get0.
    by apply (hwfst2 _ _ _ _ heq2).
  case: Mr.get (hincls sr.(sr_region)) => [sm1|] /=.
  + by case: Mr.get.
  move=> _.
  apply /Mvar.inclP => x.
...
Lemma wfr_STATUS_set_clear_pure rmap se sr ty :
  ...
  wf_sub_region se sr ty ->
  wfr_STATUS rmap se ->
  wfr_STATUS (set_clear_pure rmap sr) se.
Proof.
...
Proof.
  ...
  + case: get_local => [pk|//].
    ...
    t_xrbindP=> /check_validP hstatus _ /= {rmap2}rmap2 hclear <- _ _.
...
Lemma alloc_call_arg_aux_wfr_STATUS se (rmap0 rmap:region_map) opi e rmap2 bsr e2 :
  ...
  wfr_STATUS rmap se ->
  alloc_call_arg_aux pmap rmap0 rmap opi e = ok (rmap2, (bsr, e2)) ->
  wfr_WF rmap2 se /\ wfr_STATUS rmap2 se.
Proof.
  ...
  + case: get_local => [pk|//].
    ...
    move=> /set_clearP [hw ->].
    split=> //.
    by apply (wfr_STATUS_set_clear_pure hwfsr (hwfsr0 _ _ hsr) hwfst).
  case: get_local => [//|].
  by t_xrbindP=> _ <- _ _; split.
Qed.

Lemma alloc_call_args_aux_wfr_STATUS_aux se (rmap0 rmap:region_map) err sao_params args rmap2 l :
  wfr_WF rmap0 se ->
  wfr_WF rmap se ->
  wfr_STATUS rmap se ->
  fmapM2 err (alloc_call_arg_aux pmap rmap0) rmap sao_params args = ok (rmap2, l) ->
...
Lemma alloc_call_args_aux_wfr_STATUS se rmap sao_params args rmap2 l :
  wfr_WF rmap se ->
  wfr_STATUS rmap se ->
  alloc_call_args_aux pmap rmap sao_params args = ok (rmap2, l) ->
  wfr_STATUS rmap2 se.
...
Lemma alloc_call_args_aux_uincl table rmap se m0 s1 s2 sao_params args rmap2 l wdb vargs1 :
  valid_state table rmap se m0 s1 s2 ->
  alloc_call_args_aux pmap rmap sao_params args = ok (rmap2, l) ->
...
Lemma alloc_call_arg_aux_wf table rmap0 rmap se m0 s1 s2 opi e1 rmap2 e2 wdb vargs vargs' wptrs aligns i :
  valid_state table rmap0 se m0 s1 s2 ->
...
Lemma alloc_call_args_aux_wf table rmap se m0 s1 s2 sao_params args rmap2 l wdb vargs1 vargs2 :
  valid_state table rmap se m0 s1 s2 ->
  alloc_call_args_aux pmap rmap sao_params args = ok (rmap2, l) ->
  sem_pexprs wdb gd s1 args = ok vargs1 ->
  sem_pexprs wdb [::] s2 (map snd l) = ok vargs2 ->
...
Lemma alloc_call_arg_aux_sub_region table rmap0 rmap se m0 s1 s2 opi e1 rmap2 bsr e2 wdb v1 v2 :
  valid_state table rmap0 se m0 s1 s2 ->
  alloc_call_arg_aux pmap rmap0 rmap opi e1 = ok (rmap2, (bsr, e2)) ->
  sem_pexpr wdb gd s1 e1 = ok v1 ->
  sem_pexpr wdb [::] s2 e2 = ok v2 -> [/\
    forall b sr, bsr = Some (b, sr) ->
      wf_sub_region se sr (type_of_val v1)
      /\ (forall addr, sub_region_addr se sr = ok addr -> v2 = Vword addr) &
    forall sr, bsr = Some (true, sr) ->
...
Lemma incl_interval_add_sub_interval_swap i s1 s2 i1 i2 :
  add_sub_interval i s1 = Some i1 ->
  add_sub_interval i1 s2 = Some i2 ->
  exists i2' i1', [/\
    add_sub_interval i s2 = Some i2',
    add_sub_interval i2' s1 = Some i1' &
    incl_interval i1' i2].
Proof.
  elim: i i1 i2 => [|s i ih] i1 i2 /=.
  + move=> [<-] h. exists [::s2].
    move: h. rewrite /add_sub_interval.
    rewrite symbolic_slice_beq_sym.
    case heq: symbolic_slice_beq.
    + move=> [<-]. eexists _; split; [reflexivity..|] => /=. rewrite symbolic_slice_beq_sym heq. done.
    case hle1: (odflt _ _).
    + move=> [<-].
      case hle2: (odflt _ _).
      + eexists _; split. reflexivity. reflexivity. simpl. rewrite !symbolic_slice_beq_refl /= orbT. done.
      eexists _; split. reflexivity. reflexivity. apply incl_interval_refl.
    case hle2: (odflt _ _).
    + move=> [<-].
      eexists _; split. reflexivity. reflexivity. apply incl_interval_refl.
    done.
  case heq: symbolic_slice_beq.
  + move=> [<-] /=.
    case heq2: symbolic_slice_beq.
    + move=> [<-]. exists (s :: i). simpl. rewrite heq. eexists _. split. reflexivity. reflexivity. apply incl_interval_refl.
    case hle2: (odflt _ _).
    + move=> [<-].
      exists [:: s2, s & i] => /=.
      case heq3: symbolic_slice_beq.
      + eexists _. split. reflexivity. reflexivity. apply incl_interval_refl.
      case hle2': (odflt _ _).
      + eexists _. split. reflexivity. reflexivity.
        simpl. rewrite heq !symbolic_slice_beq_refl ?orbT /=.
        do 2 apply: incl_interval_trans (incl_interval_cons _ _).
        apply incl_interval_refl.
      have ->: symbolic_slice_ble s2 s1 = symbolic_slice_ble s2 s.
      + rewrite <- (heq : is_true _). done.
      rewrite hle2 heq.
      eexists _. split. reflexivity. reflexivity. apply incl_interval_refl.
    case hle2': (odflt _ _).
    + apply: obindP => {}i2 hadd2 [<-]. rewrite hadd2.
      exists (s::i2) => /=.
      rewrite heq. eexists _; split. reflexivity. reflexivity. apply incl_interval_refl.
    done.
  case hle1: (odflt _ _).
  + move=> [<-] /=.
    case heq2: symbolic_slice_beq.
    + move=> [<-].
      have ->: symbolic_slice_beq s2 s = false.
      + rewrite -> (heq2 : is_true _). done.
      have ->: (symbolic_slice_ble s2 s) = (symbolic_slice_ble s1 s).
      +  rewrite -> (heq2 : is_true _). done.
      rewrite hle1.
      exists [:: s2, s & i]. simpl.
      rewrite (symbolic_slice_beq_sym' heq2).
      eexists _. split. reflexivity. reflexivity. simpl. rewrite heq2 !symbolic_slice_beq_refl !orbT /=.
      do 2 apply: incl_interval_trans (incl_interval_cons _ _).
      apply incl_interval_refl.
    case hle2: (odflt _ _).
    + move=> [<-].
      ...
      + exists (s::i). simpl.
        rewrite heq hle1.
        eexists _. split. reflexivity. reflexivity.
        rewrite /= (symbolic_slice_beq_sym' heq3) symbolic_slice_beq_refl orbT /=.
        do 3 apply: incl_interval_trans (incl_interval_cons _ _).
        apply incl_interval_refl.
      case hle3: (odflt _ _).
      + exists (s2::s::i). simpl.
        rewrite symbolic_slice_beq_sym heq2.
        case hle4: (odflt _ _).
        + eexists _. split. reflexivity. reflexivity.
          simpl. rewrite !symbolic_slice_beq_refl !orbT /=.
          do 3 apply: incl_interval_trans (incl_interval_cons _ _).
          apply incl_interval_refl.
        rewrite hle2 heq hle1.
        eexists _. split. reflexivity. reflexivity. apply incl_interval_refl.
      case hle4: (odflt _ _).
      + admit. (*
...
Lemma incl_status_clear_status_swap status z1 z2 status1 status2 :
  clear_status status z1 = Some status1 ->
  clear_status status1 z2 = Some status2 ->
  exists status2' status1', [/\
    clear_status status z2 = Some status2',
    clear_status status2' z1 = Some status1' &
    incl_status status2 status1'].
Proof.
  case: z1 => [//|s1 _] /=.
  case: z2 => [//|s2 _] /=.
  case: status => [||i].
  ...
  + done.
  apply: obindP=> i1 hadd1 [<-].
  apply: obindP=> i2 hadd2 [<-].
  have := incl_interval_add_sub_interval_swap hadd1 hadd2. move=> [i2' [i1' [hadd2' hadd1' hincl]]].
  rewrite hadd2'. exists (Borrowed i2').
  rewrite /= hadd1'.
  eexists _. split. reflexivity. reflexivity. done.
Qed.

Lemma incl_set_clear_pure_swap rmap sr1 sr2 :
  incl (set_clear_pure (set_clear_pure rmap sr1) sr2)
       (set_clear_pure (set_clear_pure rmap sr2) sr1).
Proof.
  apply /andP; split.
  + apply /Mvar.inclP => x /=.
    case: Mvar.get => [sr|//].
  ...
  rewrite /set_clear_status /get_status_map /set_clear_pure /= /clear_status_map /=.
  rewrite /clear_status_map_aux /=.
  ...
  + case: eqP => heqr1 /=.
    + apply /Mvar.inclP => x.
      rewrite !Mvar.filter_mapP. rewrite heqr2 heqr1 eq_refl /=.
      rewrite /clear_status_map !Mvar.filter_mapP.
      rewrite /clear_status_map_aux /=.
      case: Mvar.get => /=; last first.
      + case: Mvar.get => //.
      move=> sr. case: Mvar.get => //= status.
      case: (get_suffix _ sr1.(sr_zone)) (get_suffix _ sr2.(sr_zone)) => [[z1|]|] [[z2|]|] //=.
      case hstatus1: clear_status => [status1|//] /=.
      case hstatus2: clear_status => [status2|//] /=.
      have [? [? [-> /= -> /=]]] := incl_status_clear_status_swap hstatus1 hstatus2. done.
      case: clear_status => //= ?; by apply incl_status_refl.
      case: clear_status => //.
      case: clear_status => //= ?; by apply incl_status_refl.
      apply incl_status_refl. (*
       /=; last first.
      case: oz1 => //= ?.
      case: clear_status => //=.
      move=> oz2.
      case: oz1 oz2 => [|]
       move=> [] //=. (get_suffix _ sr2.(sr_zone)) => [[|]|] [[|]|] //=.
      case: get_suffix => [oz1|] //=.
      case: oz2 => [z2|] /=.
       _ _ with | Some (Some _) => _ | _ => _ end) => [status1|//] /=.
      case h2: (match get_suffix _ _ with | Some (Some _) => _ | _ => _ end) => [status2|//] /=.
      rewrite h2.
  ...
  case: eqP => heqr2.
  + case: eqP => heqr1.
    + apply /Mvar.inclP => x. rewrite /clear_status_map Mvar.filter_mapP.
      rewrite /get_status_map. rewrite /=. rewrite Mr.setP.
      rewrite heqr1 heqr2 eq_refl /=. rewrite Mvar.filter_mapP.
      rewrite Mr.setP heqr2 eq_refl /=.
      admit.
    apply /Mvar.inclP => x.
    rewrite /clear_status_map.
    rewrite !Mvar.filter_mapP. rewrite get_status_map_setP.
    case: eqP. congruence. move=> _. case: Mvar.get => //= status.
    rewrite /clear_status_map_aux /=.
    case: (match _ with | Some (Some _) => _ | _ => _ end) => //. move=> ?; apply incl_status_refl.
  case: eqP.
  + move=> heqr1.
    apply /Mvar.inclP => x.
    rewrite !Mvar.filter_mapP.
    rewrite get_status_map_setP.
    case: eqP; try congruence. move=> _.
    case: Mvar.get => //= status.
    rewrite /clear_status_map_aux /=.
    case: (match _ with | Some (Some _) => _ | _ => _ end) => //. move=> ?; apply incl_status_refl.
    move=> _. case: Mr.get => // ?. by apply incl_status_map_refl. *)
Admitted.
...
Definition sub_region_cleared (rmap:region_map) se sr :=
  forall x off,
    valid_offset se (get_var_status rmap sr.(sr_region) x) off ->
    exists srx, Mvar.get rmap.(var_region) x = Some srx /\
      forall cs csx,
        sem_zone se sr.(sr_zone) = ok cs ->
        sem_zone se srx.(sr_zone) = ok csx ->
        0 <= off < csx.(cs_len) ->
...
Lemma set_clear_sub_region_cleared rmap se sr ty :
  ...
  wfr_STATUS rmap se ->
  wf_sub_region se sr ty ->
  sub_region_cleared (set_clear_pure rmap sr) se sr.
Proof.
  ...
  have hwfs := get_var_status_wf_status sr.(sr_region) x hwfst.
  have:= hwf.(wfsr_zone); rewrite /wf_zone ok_cs => -[_ [<-] wf_cs].
  by have [_ +] :=
       valid_offset_clear_status_map_aux hwfs hsrx ok_csx ok_cs
         (wf_concrete_slice_len_gt0 wf_cs) hoff off_valid.
Qed.

(* inclusion of var_region in opposite direction to region_var *)
Lemma incl_sub_region_cleared rmap1 rmap2 se sr :
  Mvar.incl (fun _ => sub_region_beq) rmap2.(var_region) rmap1.(var_region) ->
  Mr.incl (fun _ => incl_status_map) rmap1.(region_var) rmap2.(region_var) ->
  wfr_STATUS rmap1 se ->
  sub_region_cleared rmap2 se sr ->
  sub_region_cleared rmap1 se sr.
Proof.
  move=> hinclr hincls hwfst1 hcleared2 x off off_valid1.
  have hwfs := get_var_status_wf_status sr.(sr_region) x hwfst1.
  have {}hincls:
    incl_status
  ...
  rewrite hsrx2.
  case hsrx1: Mvar.get => [srx1|//] heqsub.
  exists srx1; split=> //.
  move=> cs csx ok_cs ok_csx hoff.
  apply: (hcleared2 _ _ ok_cs _ hoff).
  case/andP: heqsub => _ heqzone.
  by rewrite (symbolic_zone_beq_sem_zone se heqzone).
Qed.

Lemma alloc_call_args_aux_sub_region table rmap se m0 s1 s2 sao_params args rmap2 l wdb vargs1 vargs2 :
  valid_state table rmap se m0 s1 s2 ->
  alloc_call_args_aux pmap rmap sao_params args = ok (rmap2, l) ->
  sem_pexprs wdb gd s1 args = ok vargs1 ->
  sem_pexprs wdb [::] s2 (map snd l) = ok vargs2 -> [/\
...
Proof.
  move=> hvs.
  have: wfr_STATUS rmap se by apply wfr_status.
  have: wfr_WF rmap se by apply wfr_wf.
...
(* TODO: move? *)
Lemma allP' {T : Type} {a : pred T} {s : seq T} :
  reflect (forall x, List.In x s -> a x) (all a s).
Proof.
  elim: s => [|x s ih] /=.
  + by constructor.
  case: (@idP (a x)) => hx /=; last first.
  + constructor => contra.
    by apply hx; apply contra; left.
  apply: (iffP ih).
  + by move=> {}ih y [<-|/ih].
  by move=> h ??; apply h; right.
Qed.

(* probably a better proof exists *)
Lemma nth_List_nth (T : Type) (x0 : T) (s : seq T) (n : nat) :
  nth x0 s n = List.nth n s x0.
...
Lemma check_all_disjP notwritables writables srs :
  ...
  forall b1 sr1 sr2, List.In (Some (b1, sr1)) (map fst srs) -> List.In sr2 writables -> disj_sub_regions sr1 sr2,
  forall sr1 sr2, List.In (Some (true, sr1)) (map fst srs) -> List.In sr2 notwritables -> disj_sub_regions sr1 sr2 &
...
Proof.
  ...
  + move=> /ih [ih1 ih2 ih3].
    ...
    + move=> hnth1 [_ <-] _.
      apply ih2; last by move=> /=; left.
      rewrite -hnth1 nth_List_nth.
      apply List.nth_In; apply /ltP.
  ...
  + case: i2 => [//|i2].
    ...
    apply (ih1 b2); last by move=> /=; left.
    rewrite -hnth2 nth_List_nth.
    apply List.nth_In; apply /ltP.
    by apply (nth_not_default hnth2 ltac:(discriminate)).
  move=> /= hnth1.
  case: i2 => [|i2].
  + move=> [_ <-] _.
    apply (ih1 true); last by move=> /=; left.
    rewrite -hnth1 nth_List_nth.
    apply List.nth_In; apply /ltP.
    by apply (nth_not_default hnth1 ltac:(discriminate)).
  move=> /= hnth2 hneq.
...
(* TODO: remove disjoint_symbolic_zone and express everything in terms of
   concrete_slice? *)
Lemma disjoint_zonesP se z1 z2 :
  disjoint_zones z1 z2 ->
  disjoint_symbolic_zone se z1 z2.
Proof.
  elim: z1 z2 => [|s1 z1 ih1] [|s2 z2] //=.
  ...
  + move=> hdisj.
    ...
    have z2_nnil: z2 <> [::].
    + by case: (z1) (z2) hdisj => [|??] [|??] /=.
    rewrite /disjoint_symbolic_zone /= -(symbolic_slice_beqP se heq).
...
Lemma disjoint_symbolic_zone_disjoint_zrange se sr1 ty1 sr2 ty2 addr1 addr2 :
  wf_sub_region se sr1 ty1 ->
  sub_region_addr se sr1 = ok addr1 ->
  wf_sub_region se sr2 ty2 ->
  sub_region_addr se sr2 = ok addr2 ->
  sr1.(sr_region) = sr2.(sr_region) ->
  disjoint_symbolic_zone se sr1.(sr_zone) sr2.(sr_zone) ->
...
Lemma disj_sub_regions_disjoint_zrange se sr1 sr2 ty1 ty2 addr1 addr2 :
  wf_sub_region se sr1 ty1 ->
  sub_region_addr se sr1 = ok addr1 ->
  wf_sub_region se sr2 ty2 ->
  sub_region_addr se sr2 = ok addr2 ->
...
Proof.
  ...
  case: eqP => heqr /=.
  + move=> /(disjoint_zonesP (se:=se)) hdisj.
...
Lemma disj_sub_regions_disjoint_values se (srs:seq (option (bool * sub_region))) sao_params vargs1 vargs2 :
  ...
  Forall3 (fun bsr varg1 varg2 => forall (b:bool) (sr:sub_region), bsr = Some (b, sr) ->
    wf_sub_region se sr (type_of_val varg1)
    /\ (forall addr, sub_region_addr se sr = ok addr -> varg2 = Vword addr)) srs vargs1 vargs2 ->
  forall i, nth None (map (omap pp_writable) sao_params) i = Some true ->
    forall j vai vaj (pi pj : word Uptr),
    i <> j ->
    isSome (nth None (map (omap pp_writable) sao_params) j) ->
...
Proof.
  ...
  have /ltP /(List.nth_In _ None) := nth_not_default hsri ltac:(discriminate).
  rewrite -nth_List_nth => hini.
...
(* Full spec *)
Lemma alloc_call_argsP table rmap se m0 s1 s2 sao_params args rmap2 l wdb vargs1 :
  valid_state table rmap se m0 s1 s2 ->
  alloc_call_args pmap rmap sao_params args = ok (rmap2, l) ->
  sem_pexprs wdb gd s1 args = ok vargs1 ->
  exists vargs2, [/\
...
Lemma mem_unchanged_holed_rmap se m0 s1 s2 mem1 mem2 l :
  valid_incl m0 (emem s2) ->
  validw (emem s1) =3 validw mem1 ->
  List.Forall (fun '(sr, ty) => wf_sub_region se sr ty /\ sr.(sr_region).(r_writable)) l ->
  (forall p,
...
Lemma eq_read_holed_rmap table rmap se m0 s1 s2 mem2 l sr ty addr off :
  valid_state table rmap se m0 s1 s2 ->
  List.Forall (fun '(sr, ty) => wf_sub_region se sr ty /\ sr.(sr_region).(r_writable)) l ->
  (forall p,
    validw (emem s2) Aligned p U8 -> ~ validw (emem s1) Aligned p U8 ->
    List.Forall (fun '(sr, ty) =>
      ...
      disjoint_zrange addr (size_of ty) p (wsize_size U8)) l ->
    read mem2 Aligned p U8 = read (emem s2) Aligned p U8) ->
  List.Forall (fun '(sr, ty) => sub_region_cleared rmap se sr) l ->
  wf_sub_region se sr ty ->
  sub_region_addr se sr = ok addr ->
...
Lemma wfr_VAL_holed_rmap table rmap se m0 s1 s2 mem1 mem2 l :
  valid_state table rmap se m0 s1 s2 ->
  List.Forall (fun '(sr, ty) => wf_sub_region se sr ty /\ sr.(sr_region).(r_writable)) l ->
  (forall p,
    validw (emem s2) Aligned p U8 -> ~ validw (emem s1) Aligned p U8 ->
...
Lemma wfr_PTR_holed_rmap table rmap se m0 s1 s2 mem2 l :
  valid_state table rmap se m0 s1 s2 ->
  List.Forall (fun '(sr, ty) => wf_sub_region se sr ty /\ sr.(sr_region).(r_writable)) l ->
  (forall p,
    validw (emem s2) Aligned p U8 -> ~ validw (emem s1) Aligned p U8 ->
...
Proof.
  ...
  have /wf_locals /= hlocal := hlx.
  have hwfs := sub_region_stkptr_wf se hlocal.
  rewrite !(read8_alignment Aligned).
...
Lemma valid_state_holed_rmap table rmap se m0 s1 s2 mem1 mem2 l :
  valid_state table rmap se m0 s1 s2 ->
  validw (emem s1) =3 validw mem1 ->
  stack_stable (emem s2) mem2 ->
...
Proof.
  ...
  apply: (valid_state_set_move_regptr hvs _ ok_addr _ hlx) => //.
...
Proof.
  ...
  elim: {ret_pos vres1 vres2} hresults heqinmems table rmap s1 s2 hvs hmem rs rmap2 rs2 halloc s1'.
  + move=> _ table rmap s1 s2 hvs _ [|//] _ _ /= [<- <-] _ [<-].
    by eexists; split; first by reflexivity.
  move=> oi vr1 vr2 ret_pos vres1 vres2 hresult _ ih /= /List_Forall3_inv [heqinmem heqinmems]
    table rmap0 s1 s2 hvs ? [//|r rs] /=; subst m2.
  ...
  have [s2'' [hs2'' hvs'']] := ih heqinmems _ _ _ _ hvs' heqmem _ _ _ halloc _ hs1''.
  rewrite /= hs2' /= hs2'' /=.
  by eexists; split; first by reflexivity.
Qed.

Lemma check_resultP table rmap se m0 s1 s2 srs params (sao_return:option nat) res1 res2 wdb vres1 vargs1 vargs2 :
  valid_state table rmap se m0 s1 s2 ->
  ...
  List.Forall2 (fun osr varg2 => forall sr addr,
    osr = Some sr -> sub_region_addr se sr = ok addr -> varg2 = Vword addr) srs vargs2 ->
...
Proof.
  ...
  + case heq: nth => [sr|//].
    ...
    t_xrbindP=> /check_validP hvalid heqsub p /get_regptrP hlres1 <-.
    have /wfr_wf hwf := hsr.
    have [cs ok_cs _] := hwf.(wfsr_zone).
    have [addr ok_addr _] := wunsigned_sub_region_addr hwf ok_cs.
    have /wfr_ptr := hsr.
    ...
    + split; last first.
      + symmetry.
        apply (Forall2_nth haddr None (Vbool true) (nth_not_default heq ltac:(discriminate)) _ _ heq).
        by rewrite (sub_region_beq_addr se heqsub).
...
Lemma check_resultsP table rmap se m0 s1 s2 srs params sao_returns res1 res2 vargs1 vargs2 wdb :
  valid_state table rmap se m0 s1 s2 ->
  Forall3 (fun osr (x : var_i) v => osr <> None -> subtype x.(vtype) (type_of_val v)) srs params vargs1 ->
  List.Forall2 (fun osr varg2 => forall sr addr,
    osr = Some sr -> sub_region_addr se sr = ok addr -> varg2 = Vword addr) srs vargs2 ->
...
*)
Lemma valid_state_distinct_reg table rmap se m0 s1 s2 x v :
  valid_state table rmap se m0 s1 s2 ->
  x <> pmap.(vrip) ->
  x <> pmap.(vrsp) ->
  Sv.In x pmap.(vnew) ->
  (forall y p, get_local pmap y = Some (Pregptr p) -> x <> p) ->
  valid_state table rmap se m0 s1 (with_vm s2 (evm s2).[x <- v]).
Proof.
...
Lemma fill_fill_mem table rmap se m0 s1 s2 sr len addr l a :
  valid_state table rmap se m0 s1 s2 ->
  wf_sub_region se sr (sarr len) ->
  sub_region_addr se sr = ok addr ->
...
Lemma valid_state_scs table rmap se m0 s1 s2 scs :
  valid_state table rmap se m0 s1 s2 ->
  valid_state table rmap se m0 (with_scs s1 scs) (with_scs s2 scs).
Proof.
  move=> hvs.
  case:(hvs) => hscs hvalid hdisj hincl hincl2 hunch hrip hrsp heqvm hwft hwfr heqmem hglobv htop.
  constructor=> //=.
...
Proof.
  case: z => [//|s _] /=.
  case: status => [||i] /=.
  + by rewrite symbolic_slice_beq_refl /= symbolic_slice_beq_refl.
  + by [].
...
Proof.
  ...
  have hvs': valid_state table rmap se m0 s1 s2'.
  ...
  have hwfst: wfr_STATUS rmap2 se.
  + move /set_clearP : hrmap2 => [_ ->].
    by apply (wfr_STATUS_set_clear_pure wfr_wf hwfg wfr_status).
  have hvs2 := valid_state_Incl hincl hwfst hvs'.
  ...
  have hvs2'': valid_state table rmap2 se m0 s1'' s2''.
  + by apply valid_state_scs.

  (* write the result *)
  set s1''' := with_vm s1'' (evm s1'').[x <- Varr a2].
  set s2''' := with_vm s2'' (evm s2'').[px <- Vword addrg].
  have hvs2''': valid_state (remove_binding table x) (set_move rmap2 x srg Valid) se m0 s1''' s2'''.
  + rewrite /s1''' /s2'''.
    move: hwfg; rewrite -hty => hwfg.
    apply: (valid_state_set_move_regptr hvs2'' hwfg ok_addrg _ hlx h) => //.
  ...
  + rewrite /= /get_gvar /= /get_var.
    have /wfr_ptr := hgetg; rewrite /get_local hlg => -[_ [[<-] /= hpk]].
