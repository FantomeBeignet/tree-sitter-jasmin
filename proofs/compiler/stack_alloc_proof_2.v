...
Definition incl_table (table1 table2 : table) :=
  [&& Mvar.incl (λ _ : Mvar.K.t, eq_expr) (bindings table1) (bindings table2), (counter table2 ≤? counter table1)%uint63
    & Sv.subset (vars table2) (vars table1)].

(* TODO: move to stack_alloc_proof *)
Lemma valid_state_incl_table table1 table2 pmap rmap se m0 s1 s2 :
  incl_table table1 table2 ->
  valid_state pmap glob_size rsp rip Slots Addr Writable Align P table2 rmap se m0 s1 s2 ->
  valid_state pmap glob_size rsp rip Slots Addr Writable Align P table1 rmap se m0 s1 s2.
Proof.
  move=> hinclt hvs.
  case:(hvs) => hscs hvalid hdisj hincl hincl2 hunch hrip hrsp heqvm hwft hwfr heqmem hglobv htop.
  split=> //.
  have /and3P [/Mvar.inclP hinclb hle /Sv.subset_spec hsub] := hinclt.
  case: hwft => hvars hdef hsem.
  split.
  + admit. (* move=> x e hget1.
    case: Mvar.get hget1 (hinclb x) => [_|//] [->].
    case hget2: Mvar.get => [{}e|//] /eqP ->.
    move: (hvars _ _ hget2). clear -hsub. SvD.fsetdec.
    move: hvars; rewrite /wft_VARS.
    apply: SvD.F.Subset_trans hsub.
    by apply: hvars hget2. *)
  + move=> x hnin.
    apply hdef. apply hsub. done.
  move=> x e v1 hget1 ok_v1.
  case: Mvar.get hget1 (hinclb x) => [_|//] [->].
  case hget2: Mvar.get => [e2|//] heq.
  have [v2 ok_v2 huincl] := hsem _ _ _ hget2 ok_v1.
  exists v2 => //.
  move: heq => /eqP ->. done.
  by rewrite (eq_exprP _ _ _ heq).
Qed.
*)
...
Proof.
  ...
  + apply /Mvar.inclP => x.
    ...
    by case: eqP.
  + case: ifP.
    + move=> _. apply /lebP. lia.
    move=> /lebP ?. apply /lebP. lia.
  apply Sv.subset_spec. clear; SvD.fsetdec.
Qed.
...
Lemma incl_table_remove_binding table x :
  incl_table (remove_binding table x) table.
Proof.
  apply /and3P; split=> /=.
  + apply /Mvar.inclP => y.
  ...
  + apply /Uint63.lebP. lia.
  by apply Sv.subset_spec.
Qed.

Lemma remove_binding_empty_table x :
  incl_table empty_table (remove_binding empty_table x).
Proof. done. Qed.
*)
...
(* FIXME: what to do with this? axiom, hyp, other?? *)
...
Let Pi_r (i:instr_r) :=
  ...
  alloc_i pmap local_alloc sao (table1, rmap1) (MkI ii i) = ok (table2, rmap2, c2) ->
  wft_VARS table1 ->
  wfr_VARS_ZONE table1.(vars) rmap1 ->
...
Let Pi (i:instr) :=
  ...
  alloc_i pmap local_alloc sao (table1, rmap1) i = ok (table2, rmap2, c2) ->
  wft_VARS table1 ->
  wfr_VARS_ZONE table1.(vars) rmap1 ->
  wfr_VARS_STATUS table1.(vars) rmap1 -> [/\
    wft_VARS table2,
...
Let Pc (c1:cmd) :=
  forall table1 rmap1 table2 rmap2 c2,
  fmapM (alloc_i pmap local_alloc sao) (table1, rmap1) c1 = ok (table2, rmap2, c2) ->
  wft_VARS table1 ->
  wfr_VARS_ZONE table1.(vars) rmap1 ->
  wfr_VARS_STATUS table1.(vars) rmap1 -> [/\
    wft_VARS table2,
...
Proof.
  ...
  t_xrbindP=> -[[table1' rmap1'] ?] /Hi{}Hi [[{}table2 {}rmap2] ?] /Hc{}Hc /= [<- <-] _.
  move=> hvars1 hvarsz1 hvarss1.
  have [hvars1' hsubset1 hvarsz1' hvarss1'] := Hi hvars1 hvarsz1 hvarss1.
  have [hvars2 hsubset2 hvarsz2 hvarss2] := Hc hvars1' hvarsz1' hvarss1'.
  split=> //.
...
Lemma update_tableE table r ty oe table2 :
  update_table table r ty oe = ok table2 ->
  (exists x e, [/\ r = Lvar x, oe = Some e & table_set_var table x e = Some table2])
  \/ table2 = table.
Proof.
  case: oe => [e|] /=; last by move=> [<-]; right.
  case: r => /=; try by move=> > /ok_inj ?; right.
  t_xrbindP=> x _ /o2rP hset.
  by left; exists x, e.
Qed.

Lemma table_set_var_vars table1 x e table2 :
  table_set_var table1 x e = Some table2 ->
  table2.(vars) = table1.(vars).
Proof.
  rewrite /table_set_var.
  by case: ifP => // _ [<-] /=.
Qed.

Lemma wft_VARS_table_set_var table1 x e table2 :
  table_set_var table1 x e = Some table2 ->
  Sv.Subset (read_e e) table1.(vars) ->
  wft_VARS table1 ->
  wft_VARS table2.
Proof.
  rewrite /table_set_var.
  case: ifP => // _ [<-] hsubset hvars1 y ey /=.
  rewrite Mvar.setP.
  case: eqP => _.
  + by move=> [<-].
  by apply hvars1.
...
(*
Lemma update_table_vars table1 r ty oe table2 :
  update_table table1 r ty oe = ok table2 ->
  table2.(vars) = table1.(vars).
Proof.
  move=> /update_tableE [|-> //].
  move=> [x [e [_ _ hset]]].
  by apply (table_set_var_vars hset).
Qed.
*)
...
Proof.
  ...
  + t_xrbindP=> -[[_ _] _] [[{}table2 {}rmap2] i] hinit /[!print_trmapP]
    ...
    suff: [/\
      ...
      wfr_VARS_ZONE table2.(vars) rmap2 &
      wfr_VARS_STATUS table2.(vars) rmap2].
    + move=> [hvars2 hsubset hvarsz2 hvarss2].
    ...
    + apply wf_vars_status_insert_status.
      ...
      apply (subset_vars_wf_vars_status hsubset2).
      by apply hvarss1'.
  case hsym: (match symbolic_of_pexpr _ _ _ with | Some _ => _ | _ => _ end) => [table1' oe].
  t_xrbindP=> -[[_ _] _] table1'' hupdate _ _ [{}rmap2 r2] hlval /[!print_trmapP] -[<- <- _] [<- <- _].
  move=> hvars1 hvarsz1 hvarss1.
  have [hvars1' hsubset hread]: [/\
    wft_VARS table1',
    Sv.Subset table1.(vars) table1'.(vars) &
    forall e, oe = Some e -> Sv.Subset (read_e e) table1'.(vars)].
  + case hsym: symbolic_of_pexpr hsym => [[table e']|] [<- <-]; last by split.
    split.
    + by apply (wft_VARS_symbolic_of_pexpr hsym hvars1).
  ...
  have [] := update_tableE hupdate; last first.
  + move=> ->.
    rewrite remove_binding_lval_vars.
    split=> //.
    by apply wft_VARS_remove_binding_lval.
  move=> [x [e' [?? hset]]]; subst r oe.
  rewrite (table_set_var_vars hset) /=.
  split=> //.
  apply (wft_VARS_table_set_var hset).
  + by apply hread.
...
Qed.
(*
Lemma merge_interval_vars i1 i2 i :
  merge_interval i1 i2 = Some i ->
  Sv.Subset (read_interval i) (Sv.union (read_interval i1) (read_interval i2)).
Proof.
  rewrite /merge_interval.
  elim: i1 i2 => [|s1 i1 ih1] i2 /=.
  + move=> [<-].
    by clear; SvD.fsetdec.
  case hadd: add_sub_interval => [i2'|]; last by rewrite merge_interval_None.
  move=> /ih1.
...
Lemma loop2_invariant ii check_c2 n table rmap table' rmap' e' c1' c2':
  (forall table rmap table1 rmap1 table2 rmap2 e' c1' c2',
    ...
    wft_VARS table ->
    wfr_VARS_ZONE table.(vars) rmap ->
    wfr_VARS_STATUS table.(vars) rmap -> [/\
      wft_VARS table1,
      wft_VARS table2,
      Sv.Subset table.(vars) table1.(vars),
      Sv.Subset table.(vars) table2.(vars),
      wfr_VARS_ZONE table1.(vars) rmap1,
      wfr_VARS_ZONE table2.(vars) rmap2,
      wfr_VARS_STATUS table1.(vars) rmap1 &
      wfr_VARS_STATUS table2.(vars) rmap2]) ->
...
Proof.
  ...
  move=> + hvars hvarsz hvarss.
  have [hvars1 hvars2 hsubset1 hsubset2 hvarsz1 hvarsz2 hvarss1 hvarss2] :=
    hcheck_c2 _ _ _ _ _ _ _ _ _ hc2 hvars hvarsz hvarss.
  case: ifP => _.
  + move=> [<- <- _ _ _].
    by split.
  move=> hloop.
  have hvars3 := wft_VARS_merge_table hvars hvars2.
  have hvarsz3 := wfr_VARS_ZONE_merge hvarsz hvarsz2.
  have hvarss3 := wfr_VARS_STATUS_merge (Sv.inter table.(vars) table2.(vars)) rmap rmap2.
  have [hvars' /= hsubset' hvarsz' hvarss'] :=
    hrec _ _ hloop hvars3 (hvarsz3 _) hvarss3.
  split=> //.
  by clear -hsubset2 hsubset'; SvD.fsetdec.
Qed.

Local Lemma Wwhile a c e c': Pc c -> Pc c' -> Pi_r (Cwhile a c e c').
Proof.
  ...
  t_xrbindP=> _ _ [[table2' rmap2'] _] /Hc'{}Hc' [<- <- <- <- _ _ _].
  move=> hvars hvarsz hvarss.
  have [hvars1' hsubset1 hvarsz1' hvarss1'] := Hc hvars hvarsz hvarss.
  have [hvars2 hsubset2 hvarsz2 hvarss2] := Hc' hvars1' hvarsz1' hvarss1'.
  split=> //.
  by clear -hsubset1 hsubset2; SvD.fsetdec.
Qed.
...
Let Pi_r s1 (i1:instr_r) s2 :=
  ...
  wf_Slots Slots Addr Writable Align ->
  forall sao,
  alloc_i pmap local_alloc sao (table1, rmap1) (MkI ii1 i1) = ok (table2, rmap2, c2) ->
...
Let Pi s1 (i1:instr) s2 :=
  ...
  wf_Slots Slots Addr Writable Align ->
  forall sao,
  alloc_i pmap local_alloc sao (table1, rmap1) i1 = ok (table2, rmap2, c2) ->
  forall vme m0 s1', valid_state pmap glob_size rsp rip Slots Addr Writable Align P table1 rmap1 vme m0 s1 s1' ->
  extend_mem (emem s1) (emem s1') rip global_data ->
  wf_sao rsp (emem s1') sao ->
  exists s2' vme', [/\
...
Proof.
  ...
  have hvars := hv.(vs_wf_table).(wft_vars).
  have hvarsz := hv.(vs_wf_region).(wfr_vars_zone).
  have hvarss := hv.(vs_wf_region).(wfr_vars_status).
  have [_ hsubset _ _] := alloc_i_invariant hi hvars hvarsz hvarss.
...
Lemma wf_table_update_table table se s1 e v ty v' r s1' table' :
  ...
  update_table table r ty (Some e) = ok table' ->
  exists2 vme,
...
Lemma remove_binding_lval_test table r :
  remove_binding_lval table r = Sv.fold (fun x table => remove_binding table x) (vrv r) table.
Proof. by case: r. Qed.

IDEM
Lemma remove_binding_lval_test2 table r x :
  Mvar.get (remove_binding_lval table r).(bindings) x =
    if Sv.mem x (vrv r) then None else Mvar.get table.(bindings) x.
Proof. case: r => [^ g] //=; rewrite Mvar.removeP; case: eqP => //; case: Sv_memP => //; clear; SvD.fsetdec. Qed.
*)

(* TODO: move close to wf_table_set_var *)
(* rename wf_table_set_var into wf_table_remove_binding
     and this one into wf_table_remove_binding_lval ? *)
...
Lemma wf_table_table_set_var table1 table2 x e vm vme :
  table_set_var table1 x e = Some table2 ->
  Sv.Subset (read_e e) table1.(vars) ->
  (forall v1,
...
Proof.
  rewrite /table_set_var.
  case: Sv_memP => // hnin [<-] hsub hsem [hvars1 hundef1 hsem1].
  split=> //.
  + move=> y ey /=.
    rewrite Mvar.setP.
...
Lemma wf_table_update_table s1 s2 r ty e v v' table1 table2 oe table3 vme :
  sem_pexpr true gd s1 e = ok v ->
(*   truncate_val ty v = ok v' -> *)
  value_uincl v' v ->
  write_lval true gd r v' s1 = ok s2 ->
  match symbolic_of_pexpr (clone fresh_var_ident) table1 e with
  | Some (table, e) => (table, Some e)
  | None => (table1, None)
  end = (table2, oe) ->
  update_table (remove_binding_lval table2 r) r ty oe = ok table3 ->
...
Proof.
  move=> hv hincl hw hsym hupdate hwft1.
  case hsym: symbolic_of_pexpr hsym hupdate => [[{}table2 e']|] [<- <-]; last first.
  + move=> /= [<-].
    rewrite remove_binding_lval_vars.
    exists vme; split => //.
  ...
  exists vme'.
  have [] := update_tableE hupdate;
    last by move=> ->; rewrite remove_binding_lval_vars.
  move=> [x [_ [? [<-] hset]]]; subst r.
  split=> //.
  + apply: (wf_table_table_set_var hset hsubset' _ hwft2').
    move=> v1.
  ...
  by rewrite (table_set_var_vars hset) remove_binding_lval_vars.
Qed.
...
Proof.
  ...
  case hsym: (match symbolic_of_pexpr _ _ _ with | Some _ => _ | _ => _ end) => [table1' oe].
  t_xrbindP=> -[[_ _] _] table1'' ok_table1'' e' he1 [{}rmap2 r'] hax /= /[!print_trmapP]
    -[<- <- <-] [<- <- <-] {table2 c2} vme m0 s2 hvs hext hsao.
  ...
  have [vme' [hwft1'' vme_eq hsubset]] :=
    wf_table_update_table hv (truncate_value_uincl htr) hw hsym ok_table1'' hwft1.
  exists s2', vme'; split=> //.
...
Proof.
  ...
  have [vme' [hwft2 vme_eq hsubset]] :=
    wf_table_update_table hv v_uincl hw hsym ok_table2 hwft1.
  exists s2', vme'; split=> //.
...
(*
Lemma merge_status_l x status1 status2 status se :
  merge_status x status1 status2 = Some status ->
  wf_status se (odflt Unknown status1) ->
  wf_status se status.
Proof.
  case: status1 => [status1|//].
  case: status2 => [status2|//] /=.
...
Lemma wfr_STATUS_merge rmap1 rmap2 se :
  wfr_STATUS rmap1 se ->
  wfr_STATUS (merge rmap1 rmap2) se.
Proof.
  move=> hwfst1.
  move=> r sm x status /=.
  rewrite Mr.map2P //. (*
  
  move /wfr_STATUS_alt in hwfst1; apply wfr_STATUS_alt.
  move=> r x.
  rewrite /get_var_status /= /get_status_map.
  rewrite Mr.map2P //. *)
  case hget1: Mr.get => [sm1|//].
  case hget2: Mr.get => [sm2|//] /=.
  case: ifP => // _ [<-].
(*   case: Mvar.is_empty => //=.
  rewrite /get_status. *)
  rewrite Mvar.map2P //.
...
(*
Lemma wf_rmap_merge pmap Slots Addr Writable Align vars rmap1 rmap2 vme s2 s2' vars1 vars2:
  wft_DEF vars vme ->
  wfr_VARS_ZONE vars2 rmap2 ->
  Sv.Subset vars vars1 ->
  wf_rmap pmap Slots Addr Writable Align P vars rmap1 vme s2 s2' ->
  wf_rmap pmap Slots Addr Writable Align P vars (merge vars rmap1 rmap2) vme s2 s2'.
Proof.
  move=> hdef hvarsz hsubset /[dup] hwfr1 [hwfsr1 hwfst1 hvarsz1 hvarss1 hval1 hptr1].
  split=> //.
  + move=> x sr /=.
    rewrite Mvar.map2P //.
    case hsr1: Mvar.get => [sr1|//].
    case: Mvar.get => [sr2|//].
    case: eqP => // _ [<-].
    by apply (hwfsr1 _ _ hsr1).
  + move=> r x.
    rewrite /get_var_status /=.
    rewrite /get_status_map Mr.map2P //.
    case hsm1: Mr.get => [sm1|//].
    case hsm2: Mr.get => [sm2|//]. simpl.
    case: ifP => //= _.
    rewrite /get_status Mvar.map2P //.
    case hst1: Mvar.get => [status1|//].
    case hst2: Mvar.get => [status2|//]. simpl.
    case: status1 status2 hst1 hst2 => [||i1] [||i2] hst1 hst2 //=.
    + admit.
    + admit.
    case hmerge: merge_interval => [i|//] /=.
    case: ifP => //= hall.
    elim: (i) hall.
    + simpl. exists [::]. done.
    move=> s i' ih /= /andP [/andP [/Sv.subset_spec h1 h2] /ih].
    have [cs ok_cs] := typecheck_sliceP h2 (subset_vars_wft_DEF h1 hdef).
    move=> [ci ok_ci].
    rewrite /wf_interval. rewrite /= ok_cs ok_ci /=. eexists; reflexivity.
  + move=> x sr /=.
    rewrite Mvar.map2P //.
    case hsr1: Mvar.get => [sr1|//].
    case: Mvar.get => [sr2|//].
    case: eqP => // _ [<-].
    have := (hvarsz1 _ _ hsr1). rewrite /wf_vars_zone. admit. 
  + move=> r x.
    rewrite /get_var_status /=.
    rewrite /get_status_map Mr.map2P //.
    case hsm1: Mr.get => [sm1|//].
    ...
    case: ifP => //= hall.
    elim: (i) hall.
    + simpl. rewrite /wf_vars_interval /=. clear; SvD.fsetdec.
    move=> s i' ih /= /andP [/andP [/Sv.subset_spec h1 _] /ih].
    rewrite /wf_vars_interval /=. clear -h1; SvD.fsetdec.
  + move=> x sr status v hgvalid.
    have := Incl_check_gvalid (incl_Incl (incl_merge_l vars rmap1 rmap2)) hgvalid.
    move=> [status1 hgvalid1 hincl].
    have := hval1 _ _ _ _ hgvalid1.
    move=> /[apply] -[hread hty].
    split=> // off addr w ok_addr off_valid ok_w.
    apply: (hread _ _ _ ok_addr _ ok_w).
    apply: (incl_statusP hincl _ off_valid). admit.
  admit.
Admitted. *)
...
(*
Lemma wf_interval_merge_interval vme i1 i2 i :
  merge_interval i1 i2 = Some i ->
  wf_interval vme i1 ->
  wf_interval vme i2 ->
  wf_interval vme i.
Proof.
  ...
  + by move=> [<-].
 case hadd: add_sub_interval => [i2'|]; last by rewrite merge_interval_None.
  move=> /ih1{}ih1 wf_i1 wf_i2.
  have [cs ok_cs {}wf_i1]:
    exists2 cs, sem_slice vme s1 = ok cs & wf_interval vme i1.
  + case: wf_i1 => /=.
    t_xrbindP=> _ cs ok_cs ci1 ok_ci1 _.
    exists cs => //.
    by exists ci1.
  have wf_i2' := wf_interval_add_sub_interval hadd wf_i2 ok_cs.
  by apply ih1.
Qed.
*)
...
Proof.
  ...
  have hvars1 := hvs.(vs_wf_table).(wft_vars).
  have hvarsz1 := hvs.(vs_wf_region).(wfr_vars_zone).
  have hvarss1 := hvs.(vs_wf_region).(wfr_vars_status).
  have hwft1' := hvs'.(vs_wf_table).
  have hvars1' := hwft1'.(wft_vars).
  have hvarsz1' := hvs'.(vs_wf_region).(wfr_vars_zone).
  have hvarss1' := hvs'.(vs_wf_region).(wfr_vars_status).
  have [hvars1'' hsubset hvarsz1'' hvarss1''] :=
    alloc_is_invariant hc2 hvars1 hvarsz1 hvarss1.
...
Proof.
  ...
  have hvarss1'' := hvs'.(vs_wf_region).(wfr_vars_status).
  have [hvars1' hsubset hvarsz1' hvarss1'] :=
    alloc_is_invariant hc1 hvars1 hvarsz1 hvarss1.
  apply: valid_state_Incl_gen hvs'.
...
Lemma loop2P ii check_c2 n table rmap table' rmap' e' c1' c2' :
  ...
  exists table1 rmap1 table2 rmap2, [/\
    incl_table table1 table, wft_VARS table1, Sv.Subset table.(vars) table1.(vars),
    Incl rmap1 rmap, wfr_VARS_ZONE table1.(vars) rmap1, wfr_VARS_STATUS table1.(vars) rmap1,
      rmap1 = rmap /\ table1 = table \/ (forall vme, wft_DEF table1.(vars) vme -> wfr_STATUS rmap1 vme),
...
Proof.
  ...
  + move=> /andP [hincl1 hincl2] [<- <- <- <- <-].
    ...
    + by apply Incl_refl.
    by left; split; reflexivity.
  move=> _ hloop.
  have hvars2' := wft_VARS_merge_table hvars hvars2.
  have hvarsz2' := wfr_VARS_ZONE_merge hvarsz hvarsz2.
  have hvarss2' := wfr_VARS_STATUS_merge (Sv.inter table.(vars) table2.(vars)) rmap rmap2.
  have
    [table3 [rmap3 [table4 [rmap4
      [hinclt3 hvars3 hsubset3 hinclr3 hvarsz3 hvarss3 hdef3 {}hc2 hinclt4 hinclr4]]]]] :=
    hrec _ _ hloop hvars2' (hvarsz2' _) hvarss2'.
  exists table3, rmap3, table4, rmap4; split=> //.
  + by apply (incl_table_trans hinclt3); apply incl_table_merge_table_l.
  + move: hsubset2 hsubset3 => /=.
    by clear; SvD.fsetdec.
...
Proof.
  ...
  + clear -print_trmapP.
    ...
    move=> hvars hvarsz hvarss.
    have [hvars1' hsubset1 hvarsz1' hvarss1'] := alloc_is_invariant hc1 hvars hvarsz hvarss.
    have [hvars2 hsubset2 hvarsz2 hvarss2] := alloc_is_invariant hc2 hvars1' hvarsz1' hvarss1'.
    split=> //.
    by clear -hsubset1 hsubset2; SvD.fsetdec.
  have hwft1 := hvs.(vs_wf_table).
  have hvars1 := hwft1.(wft_vars).
  ...
  have
    [table2 [rmap2 [table4 [rmap4
      [hinclt2 hvars2 hsubset2 hinclr2 hvarsz2 hvarss2 hdef2 hc2 hinclt4 hinclr4]]]]] :=
    loop2P hcheck_c2 hloop hvars1 hvarsz1 hvarss1.
  ...
  + have [_ + _ _] := alloc_is_invariant hc1 hvars2 hvarsz2 hvarss2.
...
Proof.
  move=> s1 s2 a c1 e c2 _ Hc1 Hv pmap rsp Slots Addr Writable Align
    table1 rmap1 table3 rmap3 ii c hpmap hwf sao /=.
  set check_c2 := (X in loop2 _ X _ _ _).
  t_xrbindP=> -[[_ _] _] [[{}table3 {}rmap3] [[e' c1'] c2']] hloop /[!print_trmapP]
    -[<- <- <-] [<- <- <-] {c}.
